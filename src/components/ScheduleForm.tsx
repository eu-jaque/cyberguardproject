...
import { Expert } from "../pages/Experts"; // Importe a tipagem que está no seu arquivo principal
  ...
  return (
    <section className="py-16">
      <div className="max-w-[1100px] mx-auto px-[2%]">
        <button onClick={onBack} className="flex items-center gap-2 text-primary mb-8 hover:underline font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="glass-card p-6">
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="font-display text-lg font-bold text-foreground">{expert.name}</h2>
              <p className="text-primary text-sm font-medium">{expert.area}</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">{expert.formation}</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
              ...
              {daysOfWeek.map((day, i) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                    selectedDay === i ? "btn-gold-3d text-primary-foreground" : "bg-secondary text-foreground/70 hover:bg-secondary/80"
                  }`}
                >
                  {day}
                </button>
                ...
                {timeSlots.map((time) => (
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 px-3 py-3 rounded-lg text-sm transition-colors ${
                      selectedTime === time ? "btn-gold-3d text-primary-foreground" : "bg-secondary/50 text-foreground/70 hover:bg-secondary"
                    }`}
                  >
            ...
            {selectedDay !== null && selectedTime && (
              <button onClick={handleConfirmClick} className="btn-gold-3d text-primary-foreground w-full py-3 rounded-lg text-sm font-bold mt-4">
